...
#dataDir = '/Volumes/stadler/cEvoUnpublished/2023-Julia-CellDev/homog'
setwd(codeDir)
...
dir = paste0(dataDir, "/", method, "/", setup)
...
#trees = c("tree_ss", "tree_sds", "tree_bd") # for "baseline_sampled"

# tree parameters
treeDir = file.path(dataDir, "Trees")
treeParams = read.csv(file.path(treeDir, "tree_params.csv")) 
#treeDir = file.path(dir, "filteredTrees")
#treeParams = read.csv(file.path(treeDir, "filtered_tree_params.csv"))
# add growth and turnover rates
treeParams = treeParams %>% mutate(growthRate = birthRate - deathRate,
                                   turnoverRate = deathRate / birthRate)
...
simStats = lapply(trees, function(tree) {
  ...
  stats = data.frame(matrix(nrow = nSim, ncol = 7))
  colnames(stats) = c("seed", "nEdited", "nEdits", "nBarcodes", "nCells", "barcodeDiv", "saturation")
  
  stats$seed = names(alignments)
  
  # get median number of edited sites per cell 
  stats$nEdited = sapply(alignments, function(a) {median(rowSums(a != 0)) }) 
  if (method == "Typewriter"){
    # additional column: median number of edited tapes per cell
    stats$nEditedTapes = sapply(alignments, function(a) {median(rowSums(a %>% select(ends_with("V1")) != 0)) })
  }
  
  # get number of different edits made across sites and cells
  ...
  # compute recorder saturation as the proportion of edited vs. editable sites
  nSites = sapply(alignments, function(a) {ncol(a)})
  stats$saturation = stats$nEdited / nSites
...
infPerformance = lapply(trees, function(tree) {
  ...
  for (i in 1:nrow(outputDat)) {
    ...
    for(parameter in parameters){
      ...
      if (parameter %in% c("deathRate", "deathProb", "turnoverRate")) {
...
trueTrees = sapply(trees, function(tree) {
  sapply(c(1:nSim), function(i) {
    read.tree(file = file.path(treeDir, paste0(tree, "_", i, ".newick")))
  }, simplify = F)
...
hpdTreesOutput = lapply(trees, function(tree) {
  ...
  for (i in 1:n) {
    ...
    if (!is.na(readLines(filePath)[4])) { 
      ...
      outputDat[i, "pRank1"] = hpdTrees[1, "Percent"] # check posterior proportion of most likely tree
    }
...
mccOutput = lapply(trees, function(tree) {
  ...
  # create output table
  outputDat = data.frame(matrix(nrow = n,  ncol = 7)) 
  colnames(outputDat) = c("seed", "RF", "wRF", "Nye", "shPI", "WS", "KS")
  ...
  for (i in 1:n) {
    ...
    # standard topological Robinson-Fould distance 
    outputDat[i, "RF"] = RF.dist(mccTree, trueTree, normalize = T)
    ...
    # distance in branch length distribution
    outputDat[i, "WS"] = wasserstein1d(mccTree$edge.length, trueTree$edge.length)
    outputDat[i, "KS"] = ks.test(mccTree$edge.length, trueTree$edge.length)$statistic
